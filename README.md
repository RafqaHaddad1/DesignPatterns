...
🌍 Real-Life scenario:
      - In web development, the Chain of Responsibility pattern can be seen in event handling. When an event occurs, it can be handled by the element that triggered it or be passed up the DOM tree
   until an element with a corresponding event handler is found.
...
🤔 When to use:
      1. When your program is expected to process different kind of requests in various ways. It lets you link several handlers into one chain and upon receiving a request, “ask” each handler
...
🖼️ **Example**: A light switch with states "On" and "Off" changing behavior based on current state.
